        ...
        if (command.requestOptions.uri.path.match(/\/session\/[^/]*\/screenshot/) && command.body.value) {
            if (!this.options.disableWebdriverScreenshotsReporting) {
                ...
                try { 
                    ...
                    console.log('addAttachment SUCCESS')
                } catch (err) {
                    console.log('Caught error in allure.addAttachment - calling Buffer.from(command.body.value, \'base64\') - will not addAttachment for this screenshot')
                    console.log('/START ERROR Adding Allure attachment: ----')
                    console.log(err)
                    console.log('/END   ERROR Adding Allure attachment: ----')
                }
