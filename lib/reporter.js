        ...
        if (command.requestOptions.uri.path.match(/\/session\/[^/]*\/screenshot/) && command.body.value) {
            if (!this.options.disableWebdriverScreenshotsReporting) {
                ...
                try { 
                    ...
                    console.log('Caught error in allure.addAttachment() calling Buffer.from(command.body.value, \'base64\') - RESULT: Will not attach this screenshot to the Allure report.')
                    //console.log('Error: ')
                    //console.log(err)
                }
